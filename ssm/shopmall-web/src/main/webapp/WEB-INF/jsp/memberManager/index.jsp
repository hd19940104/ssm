...
<%@include file="/WEB-INF/jsp/common/userinfo.jsp"%>
						...
						<form class="form-inline" role="form" style="float: left;">
							<div class="form-group has-feedback">
								<div class="input-group">
									<div class="input-group-addon">会员账号</div>
									<input class="form-control has-success" type="text"
										placeholder="请输入会员账号" id="queryText">
								</div>
							</div>
	...
	$(function(){
		...
		$("#index-47").css("color", "red");//添加对应的颜色
	...
	$(function(){
		...
		function pageQuery(pageno) {
			...
			$
					.ajax({
						type : "post",
						url : "${APP_PATH}/memberManager/pageQuery",
						data : jsonData,
						beforeSend : function() {
							index = layer.load(2, {
								time : 10 * 1000
						...
						success : function(result) {
							...
							if (result.success) {
								...
								/* 	content += '<td><input type="checkbox" value="'+data.id+'"></td>'; */
									...
									content += '<td>' + data.loginacct + '</td>';
									content += '<td>' + data.nickname + '</td>';
									content += '<td>' + data.loginstatus + '</td>';
									content += '<td>' + data.authstatus + '</td>';
									content += '<td>' + data.sellerstatus + '</td>';
									content += '<td>' + data.lockstatus + '</td>';
									content += '<td>' + data.loginnum + '</td>';
									content += '<td>' + data.regtime + '</td>';
									content += '<td>' + data.lastvisittime + '</td>';
									content += '<td>';
